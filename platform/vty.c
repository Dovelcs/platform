...
char vty_getc (struct vty *vty)
{
  ...
  /* If read success, return character. */
  return c;
}
static void
...
//vty_out(vty,"vty_auth: passwd=%s\n", passwd);
//vty_out(vty,"vty_auth: buf=%s\n", buf);
  if (passwd)
    {
      if (host.encrypt)
...
vty_auth (struct vty *vty, char *buf)
{
  ...
  {
      if(usrname)
      ret = strcmp (buf, usrname);
      else
      ret = 1;
      if(ret)
      {
        vty_out (vty, "%% unknown user name!%s", VTY_NEWLINE);
        return;
      }
      else
      {
...
vty_stdio (void (*atclose)())
{
  ...
  serial_fd = open(VTYSH_PATH, O_WRONLY);
  if (serial_fd < 0) {
    perror("open serial port");
    return NULL;
  }
  if (dup2(serial_fd, vty->wfd) < 0) {
    perror("dup2");
    close(serial_fd);
    return NULL;
  }
  if (!tcgetattr (0, &stdio_orig_termios))
    {
	  ...
	  #if GDB_DBG
      termios.c_lflag &= ~(ECHO | ECHONL | ICANON | IEXTEN);  //| ISIG 关闭此项用以支持ctrl+c，解决程序假死问题
      #else
