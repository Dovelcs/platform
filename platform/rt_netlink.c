...
netlink_parse_info (int (*filter) (struct sockaddr_nl *, struct nlmsghdr *,
                                   vrf_id_t),
                    struct nlsock *nl, struct platform_vrf *zvrf)
{
  int status;
  int ret = 0;
  int error;
    ...
    vrf_id_t vrf_id)
{
  ...
  if (table != RT_TABLE_MAIN && table != platformd.rtm_table_default)
    return 0;
#endif
...
int
interface_lookup_netlink (struct platform_vrf *zvrf)
{
  int ret;
...
/* Routing table read function using netlink interface.  Only called
   bootstrap time. */
int
netlink_route_read (struct platform_vrf *zvrf)
{
  int ret;
...
static int
netlink_talk (struct nlmsghdr *n, struct nlsock *nl, struct platform_vrf *zvrf)
{
  int status;
  struct sockaddr_nl snl;
  struct iovec iov = {
...
netlink_route_multipath (int cmd, struct prefix *p, struct rib *rib)
{
  ...
  struct platform_vrf *zvrf = vrf_info_lookup (rib->vrf_id);
                 ...
                 struct connected *ifc)
{
  ...
  struct platform_vrf *zvrf = vrf_info_lookup (ifp->vrf_id);
...
{
  struct platform_vrf *zvrf = (struct platform_vrf *)THREAD_ARG (thread);
  netlink_parse_info (netlink_information_fetch, &zvrf->netlink, zvrf);
  zvrf->t_netlink = thread_add_read (platformd.master, kernel_read, zvrf,
                                     zvrf->netlink.sock);
...
/* Exported interface function.  This function simply calls
   netlink_socket (). */
void
kernel_init (struct platform_vrf *zvrf)
{
...
void
kernel_terminate (struct platform_vrf *zvrf)
{
