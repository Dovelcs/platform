...
extern struct platform_privs_t zserv_privs;
...
zserv_flush_data(struct thread *thread)
{
  ...
  switch (buffer_flush_available(client->wb, client->sock))
    ...
    case BUFFER_ERROR:
      ...
      break;
    case BUFFER_PENDING:
      client->t_write = thread_add_write(platformd.master, zserv_flush_data,
      					 client, client->sock);
      break;
    case BUFFER_EMPTY:
      break;
...
zebra_server_send_message(struct zserv *client)
{
  ...
  switch (buffer_write(client->wb, client->sock, STREAM_DATA(client->obuf),
    ...
    case BUFFER_ERROR:
	 ...
	 deleted. */
      client->t_suicide = thread_add_event(platformd.master, zserv_delayed_close,
					   client, 0);
      return -1;
    case BUFFER_EMPTY:
      ...
      break;
    case BUFFER_PENDING:
      THREAD_WRITE_ON(platformd.master, client->t_write,
...
zread_ipv4_add (struct zserv *client, u_short length, vrf_id_t vrf_id)
{
  ...
  /* Table */
  rib->table=platformd.rtm_table_default;
...
{
  /* type of protocol (lib/platform.h) */
  u_char proto;
...
zebra_client_close (struct zserv *client)
{
  ...
  listnode_delete (platformd.client_list, client);
  XFREE (0, client);
}
...
zebra_client_create (int sock)
{
  ...
  /* Set table number. */
  client->rtm_table = platformd.rtm_table_default;
  ...
  listnode_add (platformd.client_list, client);
...
zebra_event (enum event event, int sock, struct zserv *client)
{
  switch (event)
    {
    case ZEBRA_SERV:
      thread_add_read (platformd.master, zebra_accept, client, sock);
      break;
    case ZEBRA_READ:
      client->t_read = 
	thread_add_read (platformd.master, zebra_client_read, client, sock);
      break;
    case ZEBRA_WRITE:
      /**/
      break;
...
{
  vty_out (vty, "table %d%s", platformd.rtm_table_default,
	   VTY_NEWLINE);
  return CMD_SUCCESS;
}
...
DEFUN (config_table, 
       ...
       "TABLE integer\n")
{
  platformd.rtm_table_default = strtol (argv[0], (char**)0, 10);
  return CMD_SUCCESS;
}
       ...
       "Client information")
{
  ...
  for (ALL_LIST_ELEMENTS_RO (platformd.client_list, node, client))
...
{
  if (platformd.rtm_table_default)
    vty_out (vty, "table %d%s", platformd.rtm_table_default,
	     VTY_NEWLINE);
  return 0;
}
...
{
  /* Client list init. */
  platformd.client_list = list_new ();
...
void
platform_zserv_socket_init (char *path)
{
#ifdef HAVE_TCP_ZEBRA
  zebra_serv ();
#else
