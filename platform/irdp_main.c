...
irdp_sock_init (void)
{
  ...
  t_irdp_raw = thread_add_read (platformd.master, irdp_read_raw, NULL, sock); 
...
int irdp_send_thread(struct thread *t_advert)
{
  ...
  irdp->t_advertise = thread_add_timer(platformd.master, irdp_send_thread, ifp, timer);
  return 0;
}
...
void process_solicit (struct interface *ifp)
{
  ...
  irdp->t_advertise = thread_add_timer(platformd.master, 
				       irdp_send_thread, 
				       ifp, 
				       timer);
}
