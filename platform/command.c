...
"\r\n\
Hello, this is " PROGNAME " (version " VERSION ").\r\n\
" COPYRIGHT "\r\n\
" GIT_INFO "\r\n";
       ...
       "Exit current mode and down to previous mode\n")
{
  switch (vty->node)
    {
    case VIEW_NODE:
      if(strcmp(vty->address, "console") == 0)
      {
		vty->node = AUTH_NODE;
        //如果是串口仅退出当前用户，不做任何操作，quit命令不可结束程序
      }
      else
      {  	
...
{
  vty_out (vty, "Quagga %s (%s).%s", VERSION, host.name?host.name:"",
	   VTY_NEWLINE);
  vty_out (vty, "%s%s%s", COPYRIGHT, GIT_INFO, VTY_NEWLINE);
  vty_out (vty, "configured with:%s    %s%s", VTY_NEWLINE,
           CONFIG_ARGS, VTY_NEWLINE);
...
DEFUN (common_telnet_port,
       common_telnet_port_cmd,
       "telnet WORD PORT",
       ...
       "TCP Port number\n")
{
  ...
  // 使用系统的telnet命令
  char cmd[128];
  u_char ucIpaddr[32] = {0};
  u_int uiPort = 0;
  u_char ucPort[32] = {0};
  if(argv[0])
  {
    strcpy(ucIpaddr, argv[0]);
  }
  else
  {
    vty_out (vty, "Insufficient arguments%s", VTY_NEWLINE);
  ...
  else
  {
    ...
    if(uiPort == VTY_PORT)
    {
        vty_out(vty, "The port is used by vty%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
    strcpy(ucPort, argv[1]);
  }

  sprintf(cmd, "telnet %s %s", ucIpaddr, ucPort);
...
{
  char c ;
  vty_out (vty, "Start to check configuration with next startup configuration file, please wait...!%s", VTY_NEWLINE);
  vty_out (vty, "Current configuration may be lost after the reboot, save current configuration? [Y/N]:%s", VTY_NEWLINE); 
  sleep(1);
  // while (1)
  // {
    c = vty_getc(vty);
  ...
  if(c == 'Y' || c == 'y')
  {
    vty_out (vty, "system will reboot, please wait...%s", VTY_NEWLINE);
  }
  else
  {
    return CMD_SUCCESS;
...
DEFUN (common_uart4_test_send,
       common_uart4_test_send_cmd,
       "uart4_test send DATA",
       "Uart4_test\n"
       "Uart4_send\n"
       "Uart4_data\n")
{
  ...
  uart4_send(ucData, strlen(ucData));
  return CMD_SUCCESS;
}

DEFUN (common_uart4_test_recv,
        common_uart4_test_recv_cmd,
        "uart4_test recv",
        "Uart4_test\n"
        "Uart4_recv\n")
{
  char cmd[128];
  u_char ucData[32] = {0};
  uart4_get_recv_buf(ucData, 32);

  vty_out (vty, "uart4 recv data: %s%s", ucData, VTY_NEWLINE);
  return CMD_SUCCESS;
}
...
cmd_init (void)
{
  ...
  host.username = XSTRDUP (MTYPE_HOST, USERNAME);
  ...
  install_element (VIEW_NODE, &common_telnet_cmd);
  install_element (VIEW_NODE, &common_reboot_cmd);
  install_element (VIEW_NODE, &common_telnet_port_cmd);
  install_element (VIEW_NODE, &common_uart4_test_send_cmd);
  install_element (VIEW_NODE, &common_uart4_test_recv_cmd);

  
  install_element (CONFIG_NODE, &config_quit_cmd);
