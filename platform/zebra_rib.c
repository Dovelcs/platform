...
static void
rib_queue_add (struct platform_t *zebra, struct route_node *rn)
{
  assert (zebra && rn);
...
static void
rib_queue_init (struct platform_t *zebra)
{
  assert (zebra);
		      ...
		      u_char distance, vrf_id_t vrf_id)
{
  ...
  struct platform_vrf *zvrf = vrf_info_get (vrf_id);
...
rib_weed_table (struct route_table *table)
{
  ...
  if (table)
      ...
      RNODE_FOREACH_RIB_SAFE (rn, rib, next)
	{
	  ...
	  if (rib->table != platformd.rtm_table_default &&
...
vrf_id_get_next (vrf_id_t vrf_id, vrf_id_t *next_id_p)
{
  ...
  struct platform_vrf *zvrf = vrf_iter2info (iter);
 ...
 */
static void
zebra_vrf_table_create (struct platform_vrf *zvrf, afi_t afi, safi_t safi)
{
  rib_table_info_t *info;
  struct route_table *table;
...
struct platform_vrf *
platform_vrf_alloc (vrf_id_t vrf_id)
{
  struct platform_vrf *zvrf;
#ifdef HAVE_NETLINK
  char nl_name[64];
  ...
  zvrf = XCALLOC (MTYPE_ZEBRA_VRF, sizeof (struct platform_vrf));
...
{
  struct platform_vrf *zvrf = vrf_info_lookup (vrf_id);
